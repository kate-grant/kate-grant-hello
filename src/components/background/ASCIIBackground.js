...
const HomeBackground = () => {
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "fixed top-0 left-0 w-full h-full bg-amber-50 transition-all duration-800 ease-in-out -z-10 pointer-events-none" }));
};
class SimplexNoise1 {
    grad3 = [
        [1, 1, 0],
        ...
        const draw = () => {
            ...
            for (let y = 0; y < canvas.height; y += 16) {
                for (let x = 0; x < canvas.width; x += 10) {
                    ...
                    const charIndex = Math.min(asciiChars.length - 1, Math.floor(value * asciiChars.length));
                    const char = asciiChars[charIndex];
                    ctx.fillStyle = "#b0d1b2";
                    ctx.fillText(char, x, y);
                }
        ...
        const draw = () => {
            ...
            for (let y = 0; y < canvas.height; y += 12) {
                for (let x = 0; x < canvas.width; x += 10) {
                    ...
                    const densityMod = (noise.noise(nx * 1.5, ny * 1.5 - time * 2) + 1) / 2;
...
const getRandomHomeBackground = () => Math.round(Math.random()) === 0 ? HomeBackground : InfoBackground1;
const AsciiBackground = ({ variant }) => {
    const [CurrentComponent, setCurrentComponent] = useState(() => variant === "home" ? getRandomHomeBackground() : InfoBackground2);
    const [PrevComponent, setPrevComponent] = useState(null);
    const [transitioning, setTransitioning] = useState(false);
    useEffect(() => {
        const newComponent = variant === "home" ? getRandomHomeBackground() : InfoBackground2;
    ...
    return (_jsxs("div", { style: {
            ...
            pointerEvents: "none",
            zIndex: -1,
        }, children: [PrevComponent && (_jsx("div", { style: {
                    position: "absolute",
                    inset: 0,
                    opacity: transitioning ? 1 : 0,
                    transition: "opacity 0.5s ease",
                }, children: _jsx(PrevComponent, {}) })), _jsx("div", { style: {
                    position: "absolute",
                    inset: 0,
                    opacity: 1,
                    transition: "opacity 0.5s ease",
                }, children: _jsx(CurrentComponent, {}) }), _jsx(SynthPanel, {}), _jsx(AnimatedSVGsContainer, {})] }));
};
