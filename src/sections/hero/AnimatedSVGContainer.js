...
import { createRetriggerGuard, initAudioUnlock, noteNameToMidi, playNote, } from "../../utils/midiAudio";
const GRAVITY = 0.1;
const FRICTION = 0.95;
const BOUNCE = -0.3;
    ...
    useEffect(() => {
        ...
        const nodes = shapes.map((shape) => {
            if (shape.type === "pill") {
                ...
                const radius = circleRadiusMin + Math.random() * (circleRadiusMax - circleRadiusMin);
                const w = radius * 2;
                const h = radius * 2;
        ...
        function checkHover() {
            ...
            nodes.forEach((d) => {
                ...
                if (inside && !hovered.has(d.id)) {
                    hovered.add(d.id);
                    if (canTrigger(d.id)) {
                        const pan = Math.max(-1, Math.min(1, (d.x / expandedWidth) * 2 - 1));
        ...
        function render() {
            groups.attr("transform", (d) => `translate(${d.x},${d.y}) rotate(${(d.angle * 180) / Math.PI})`);
            groups
                .select("rect")
                .attr("x", (d) => -d.width / 2)
                .attr("y", (d) => -d.height / 2)
